...
<div class="table-wrapper">
	<table class="aggregated-callstack ay-sort table table-strpied table-bordered">
		<thead class="ay-sticky">
			<tr>
				<th rowspan="2">Function</th>
				<th rowspan="2">Call Count</th>
				<th colspan="4">Inclusive</th>
		...
		<tfoot>
			...
			foreach($aggregated_stack as $i => $a):
				...
				?>
				<tr>
					<td data-ay-sort-weight="<?=$a['callee']?>"><a href="<?=url('function', array('request_id' => $request['id'], 'callee_id' => $a['callee_id']))?>"><?=$a['callee']?></a><?php if($a['group']):?><span class="label group g-<?=$a['group']['index']?>"><?=$a['group']['name']?></span><?php endif;?></td>
